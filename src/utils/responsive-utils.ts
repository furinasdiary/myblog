 ...
 * - 769px-1023px: 根据position和tabletSidebar配置显示单侧栏
 * - 1024px及以上: 双侧栏模式同时显示左右侧栏
 */
...
export function generateGridClasses(config: ResponsiveSidebarConfig): {
	...
	if (
		...
		if (config.tabletSidebar === "right") {
			// 平板竖屏显示右侧栏: 769-1023px [内容+右侧栏], 1024px+ [左+中+右]
			gridCols =
				"grid-cols-1 md:grid-cols-[1fr_17.5rem] lg:grid-cols-[13rem_minmax(0,1fr)_13rem] xl:grid-cols-[17.5rem_minmax(0,1fr)_17.5rem]";
		} else {
			// 平板竖屏显示左侧栏（默认）: 769-1023px [左侧栏+内容], 1024px+ [左+中+右]
			gridCols =
				"grid-cols-1 md:grid-cols-[17.5rem_1fr] lg:grid-cols-[13rem_minmax(0,1fr)_13rem] xl:grid-cols-[17.5rem_minmax(0,1fr)_17.5rem]";
		}
...
export function generateRightSidebarClasses(
	...
	if (config.isBothSidebars && config.tabletSidebar === "right") {
		...
		classes.push(
			...
			"lg:col-start-3", // 平板横屏及桌面端在第3列
		);
	} else if (config.isBothSidebars) {
		// 双侧栏+平板竖屏显示左侧栏（默认）：右侧栏在1024px+显示
		classes.push(
			"lg:block",
			"lg:row-start-1",
			"lg:row-end-3",
...
export function generateMainContentClasses(
	...
	if (
		...
		if (config.tabletSidebar === "right") {
			// 双侧栏+平板竖屏右侧栏: 竖屏内容在第1列，1024px+内容在第2列
